...
public interface BrandFeign {
    ...
    Result<PageInfo> findPage(@RequestBody(required = false) Brand brand, @PathVariable(name = "page") int page, @PathVariable(name = "size") int size);
     ...
     */
    @PutMapping(value = "/{id}")
    Result update(@RequestBody Brand brand, @PathVariable(name = "id") Integer id);
     ...
     */
    @GetMapping("/{id}")
    Result<Brand> findById(@PathVariable(name = "id") Integer id);
