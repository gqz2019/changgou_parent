...
public interface CategoryBrandFeign {
    ...
    Result<PageInfo> findPage(@RequestBody(required = false) CategoryBrand categoryBrand, @PathVariable(name = "page") int page, @PathVariable(name = "size") int size);
     ...
     */
    @PutMapping(value = "/{id}")
    Result update(@RequestBody CategoryBrand categoryBrand, @PathVariable(name = "id") Integer id);
     ...
     */
    @GetMapping("/{id}")
    Result<CategoryBrand> findById(@PathVariable(name = "id") Integer id);
